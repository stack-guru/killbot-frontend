...
import { Card, Col, Button, Row, Table } from "react-bootstrap";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    let url = `${API_URL}analytics/getWeekData`;
    axios
      .get(url)
      .then((response) => {
      ...
      .catch((err) => {
        console.log("get active user data err = ", err);
      });

    url = `${API_URL}analytics/getGraphData`;
    axios
      .get(url)
      .then((response) => {
        if (response.data.length > 0) {
      ...
      .catch((err) => {
        console.log("get graph data err = ", err);
      });
  }, []);
  ...
  return (
    <div className={styles.Dashboard}>
      ...
      <Card className="overflow-hidden">
        <Card.Header>
          <span className="text-opacity">Recent Data</span>
        </Card.Header>
        <Card.Body>
          {detailData.length > 0 && (
            <>
                ...
                <div className={styles.userInfo}>
                  <span className="me-5">
                    ...
                    <span className={styles.userCount}>
                      {detailData[0].metricValues[0].value}
                    </span>
                  </span>
                  <span className="me-5">
                    Total Users :{" "}
                    <span className={styles.userCount}>
                      {detailData[0].metricValues[1].value}
                    </span>
                  </span>
                  <span>
                    New Users :{" "}
                    <span className={styles.userCount}>
                      {detailData[0].metricValues[2].value}
                    </span>
                  </span>
                </div>
              </div>
                  ...
                  {detailData.length > 0 && (
                    <div className={styles.userInfo}>
                      <span className="me-5">
                        ...
                        <span className={styles.userCount}>
                          {secToMin(
                            detailData[0].metricValues[3].value /
                              detailData[0].metricValues[1].value
                          )}
                        </span>
                      </span>
                  ...
                  {detailData.length > 0 && (
                    <div className={styles.userInfo}>
                      <span className="me-5">
                        Event Count :{" "}
                        <span className={styles.userCount}>
                          {detailData[0].metricValues[4].value}
                        </span>
                      </span>
                    </div>
                  )}
          ...
          <br/>
          {graphData.length > 0 && <ActiveUserChart data={graphData} />}
        </Card.Body>
      </Card>
    </div>
  );
